    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); sample++){
            ...
            *channelData *= drive * range;
            ...
            //    *channelData = -1;
            // }
            // *channelData = ((*channelData * blend) + (cleanSig * (1.f - blend))) * volume;

            //SOFTCLIPPER
            *channelData = (((2.f / juce::float_Pi) * atan(*channelData) * blend) + (cleanSig * (1.f - blend))) * volume;

            // //(distorted * blend) + (clean * (1-blend))
...
juce::AudioProcessorValueTreeState& AudioPluginAudioProcessor::getState() {
    return *state;
}
